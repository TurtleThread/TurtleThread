...
def translate_text_to_morse(text):
    morse_text = ""
    for letter in text:
        morse_text += MORSE_ALPHABET[letter.upper()]
        morse_text += " "
    return morse_text

def draw_morse_code_from_text(text, needle, length, height):
    morsekode = translate_text_to_morse(text)
    draw_morse_code(morsekode, needle, length, height)

needle = Turtle()
with needle.running_stitch(30):
    draw_morse_code_from_text("Hello world", needle, 60, 200)

needle.save("hello_world.jef")
needle.save("hello_world.svg")