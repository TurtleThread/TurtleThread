...
class TestTurtleZigzagStitch:
    ...
    @pytest.mark.parametrize("density", [5, 20, 50])
    def test_zigzag_density_calculation(self, stitch_length, density):
        actual_density = ZigzagStitch.round_density(stitch_length, density)
    ...
    def test_forward_centered_zigzag_stitch(self, turtle, stitch_length, density, width, angle):
        ...
        # Test distance between stitches, not including first and last as centered
        for i in range(1, len(stitches)-2):
            x0, y0, st0 = stitches[i]
            x1, y1, st1 = stitches[i+1]
            assert st0 == st1 == STITCH # Assert all are stitches

            left_mat = np.array([[x0-x0], [y0-y0]]) # Normalized position of left stitch
            right_mat = np.array([[x1-x0], [y1-y0]]) # Normalized position of right stitch
            ...
            # TODO: Add a density check?

            assert abs(left_norm[1][0] - right_norm[1][0]) == approx(width) # Check width
    ...
    def test_forward_uncentered_zigzag_stitch(self, turtle, stitch_length, density, width, angle):
        ...
        actual_density = ZigzagStitch.round_density(stitch_length, density, center=False)

        stitches = turtle.pattern.to_pyembroidery().stitches

        # Test distance between stitches, including first and last as not centered
...
class TestTurtleSatinStitch:
    def test_start_satin_stitch_sets_stitch_type(self, turtle):
        ...
        assert isinstance(turtle._stitch_group_stack[-1], turtlethread.stitches.SatinStitch)